...
export default function Layout({title, description, children }) {
  ...
  const darkModeChangeHandler = () =>{
    dispatch({ mode: darkMode ? 'DARK_MODE_OFF' : 'DARK_MODE_ON'});
    ...
    <div>
      <Head>
        <title>{title? `${title} - Next Buyhub`: 'Next Amazona'}</title>
      ...
      <AppBar position="static" className={classes.navbar}>
        <Toolbar>
            <NextLink href="/" passHref>
                <Link><Typography className={classes.brand}>Buyhub</Typography></Link>
